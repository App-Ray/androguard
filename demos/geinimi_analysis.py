...
if tainted_string != None:
    ...
    for path in tainted_string.get_paths():
        print("\t\t =>", path.get_access_flag(), path.get_method(
        ).get_class_name(), path.get_method().get_name(), path.get_method(
        ).get_descriptor(), path.get_bb().get_name(), "%x" % (
            path.get_bb().start + path.get_idx()))

tainted_field = _x.tainted_variables.get_field("Lcom/swampy/sexpos/pos/e/k;",
                                               "b", "[B")
...
tainted_field = _x.tainted_variables.get_field("Lcom/swampy/sexpos/pos/e/p;",
                                               "a", "[[B")
if tainted_field != None:
    print("\t -->", tainted_field.get_info())
    for path in tainted_field.get_paths():
...
tainted_field = _x.tainted_variables.get_field("Lcom/swampy/sexpos/pos/a;", "g",
                                               "Ljava/lang/String;")
if tainted_field != None:
    print("\t -->", tainted_field.get_info())
    for path in tainted_field.get_paths():
...
tainted_method = _x.tainted_packages.get_method(
    "Lcom/swampy/sexpos/pos/e/q;", "a",
    "(Ljava/lang/String;)Ljava/lang/String;")
for path in tainted_method:
    print(path.get_access_flag(), path.get_method().get_class_name(
    ), path.get_method().get_name(), path.get_method().get_descriptor(
    ), path.get_bb().get_name(), "%x" % (path.get_bb().start + path.get_idx()))
